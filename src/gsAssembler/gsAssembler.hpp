...
{
    gsInfo << "gsAssembler<T>::computeDirichletDofs()\n";
    ...
    if(m_ddof.size()==0)
    {
        gsInfo << "    size of m_ddof is " << m_ddof.size()
               << ", m_ddof resized to be of " << m_system.numUnknowns() << "\n";
                                               ...
                                               const int unk_)
{
    ...
    gsInfo << "gsAssembler<T>::computeDirichletDofsIntpl()\n";
